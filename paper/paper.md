...
The transport of high-energy particles (e.g., gamma-rays) through
...
[@Terray:2020], and in particular to simulate the response of gamma
...
gamma transport engine, and is interoperable with the latter. Yet, both
...
geometry. Various mesh formats are supported, such as OBJ, STL,
GeoTIFF and [Turtle][TURTLE]/PNG [@Niess:2020]. These formats
can be used to encode the components of a detector (exported from a computer-aided design, or CAD,
scheme) or a digital elevation model (DEM) describing the surrounding
...
a digital elevation model (DEM) of the Masaya volcano, derived
...
a CAD software), as well as the study site, which is usually represented
by a digital elevation model (DEM). These data are not natively
...
[FreeCAD][FREECAD] software is able to export the detector parts as STL
...
OBJ, STL, GeoTIFF and [Turtle][TURTLE]/PNG
[@Niess:2020]. [Calzone][CALZONE] then translates this data into
[Geant4][GEANT4] objects.

## Mesh specialisation

The process of meshing a DEM with triangular facets introduces specific
issues. To optimise the geometry traversal, the [Geant4][GEANT4] software uses a
voxelisation algorithm. This method scales poorly for DEMs that
...
includes a bounding volume hierarchy (BVH) algorithm (partitioning the
...
BVH for DEMs, while voxelisation is used otherwise (i.e.,
...
gamma-spectrometry and muography) is that the
